...
class Dataset():
    """
    Building master tidy data from raw FQ FINFSTA data.
    ...
    excel_sheet_name : str
        if file_type = "xlsx", select sheet name
        
    file_path : str
        path to row data file
    """
    def __init__(self, file_type=None, excel_sheet_name=None, file_path=None):
        # read data as pandas dataframe
        if file_type == "xlsx":
            self.data = pd.read_excel(file_path, sheet_name=excel_sheet_name)
        elif file_type == "csv":
            self.data = pd.read_csv(file_path, header=0)
        elif file_type == None:
            self.data = None
            print("Instanciated empty Dataset")
        else:
            print("data_type not valid")
    
    def clean_header(self, row_drop=[0, 1], nan_sign="-"):
        """
        
        Cleaning header of raw data (delete headers, replace nan, reindex).

        Parameters
        ----------
        row_drop : list of int
            the list of dropping head row number
        
        nan_sign : value
            value which to be replaced with np.nan
        """
        self.data.drop(row_drop, axis=0, inplace=True)
        self.data.replace(nan_sign, np.nan, inplace=True)
        self.data.reset_index(drop=True, inplace=True)
    
    def groupby_firm(self):
        """
        Groupby Panel DataFrame with firm name.
