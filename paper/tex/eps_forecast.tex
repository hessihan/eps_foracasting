...
\begin{figure}
  ...
  % \begin{tablenotes}
  %   \item[(注)] Panel(A)は全1,003社の予測値系列それぞれに対してU-SRWをモデル1としたときのモデル2別のDM統計量の分布をヒストグラムで表している．Panel(B)は全1,003社のDM検定のうち，10\%, 5\%, 1\%の水準で統計的に有意な企業の割合(\%)をDM統計量の正負別に示したヒートマップである．色が濃いほど統計的に有意な企業の割合が大きい． 
  % \end{tablenotes}
  % \end{threeparttable}
  % }
\end{figure}
...
%     \centering
%     \caption{モデル1: SRW のDM検定結果のまとめ}
%     \includegraphics[keepaspectratio, width=6cm]{./img/_dm_stats_dist_MAD_y_hat_srw.pdf}
%   \end{minipage}
%   \begin{minipage}[b]{0.45\linewidth}
%     \centering
%     \caption{モデル1: SRW のDM検定結果のまとめ}
%     \includegraphics[keepaspectratio, width=6cm]{./img/_dm_p_count_MAD_y_hat_srw.pdf}
%   \end{minipage}
% \end{figure}
...
% 実績値をそのまま予測値とするRWやSRWは，実績値に基づいて算出する株価収益率(PER)を用いた戦略とみなせる．
